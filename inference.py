...
def greedy_forwards(start, logProbs, neighbors, predecessors, edges, visited_old):
    ...
    while True:
        ...
        neighs_current = neighbors.get(current, [])
        if not neighs_current:
            break

        unvisited_neighbors = [n for n in neighs_current if n not in visited_old and n not in visited]
        if not unvisited_neighbors:
            break

        max_neighbor = max(unvisited_neighbors, key=lambda n: logProbs[edges[current, n]])
        max_logProb = logProbs[edges[current, max_neighbor]]

        sumLogProb += max_logProb
...
def get_contigs_greedy(g, succs, preds, edges, nb_paths=50, len_threshold=20, use_labels=False, checkpoint_dir=None,
    ...
    while True:
        ...
        with ThreadPoolExecutor(1) as executor:
            ...
            for e, idx in enumerate(idx_edges):
                ...
                future = executor.submit(run_greedy_both_ways, src_init_edges, dst_init_edges, logProbs, succs, preds,
                                         edges, visited)
                results[(src_init_edges, dst_init_edges)] = (future, e)

            indx = 0
            for k, (f, e) in results.items():
                ...
                if len_walk_it > 2:
                    meanLogProb_it = sumLogProb_it / (
                                len_walk_it - 2)
                    try:
                        meanLogProb_scaled_it = meanLogProb_it / math.sqrt(len_contig_it)
                    except ValueError:
                        print(
                            f'{indx:<3}: src={k[0]:<8} dst={k[1]:<8} len_walk={len_walk_it:<8} len_contig={len_contig_it:<12}')
...
def inference(data_path, model_path, savedir):
    assembler='hifiasm'
    ...
    for idx, g in ds:
        ...
        with torch.no_grad():
            ...
            model = models.MyTransformerModelNet(node_features, edge_features, hidden_features, hidden_edge_features, num_gt_layers, hidden_edge_scores, batch_norm, nb_pos_enc)
        ...
        if strategy == 'greedy':
            walks = get_contigs_greedy(g, succs, preds, edges, nb_paths, len_threshold, use_labels, checkpoint_dir, load_checkpoint)
        else:
        ...
        contigs = walk_to_sequence(walks, g, reads, edges)
        ...
        save_assembly(contigs, assembly_dir, idx)
...
if __name__ == '__main__':
    ...
    model = args.model
    if not model:
        model = 'pretrained/pretrained_model.pt'

    inference(data_path=data, model_path=model, savedir=out)
