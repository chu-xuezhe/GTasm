...
class MultiHeadAttentionLayer(nn.Module):
    ...
    def propagate_attention(self, g):
        ...
        # g.send_and_recv(eids, fn.src('score', 'score'), fn.sum('score', 'z'))
...
class GraphTransformerLayer(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, num_heads, dropout=0.2, layer_norm=False, batch_norm=True, residual=True,
                 use_bias=False):
        super().__init__()
...
class GT_processor(nn.Module):
    def __init__(self, num_layers, hidden_features):
        ...
        self.convs = nn.ModuleList([
            GraphTransformerLayer(in_dim=hidden_features,out_dim=hidden_features,num_heads=8) for _ in range(num_layers)
        ])

    def forward(self, graph, h, e):
        for i in range(len(self.convs)):
            print("GT_processor_forward")
